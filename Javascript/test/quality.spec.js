...
describe("GildedTros: quality of item", () => {
  ...
  test("Quality never negative", () => {
    const items = [
      new Item("test-item-1", 0, 1),
      new Item("test-item-2", -5, 0),
      new Item("test-item-3", -10, 2),
      new Item("test-item-4", -20, 1),
      new Item("test-item-5", -20, 0),
    ];
    // copy of items array
    const appItems = items.map(
      (item) => new Item(item.name, item.sellIn, item.quality)
  ...
  test("Smelly items ('Duplicate Code', 'Long Methods', 'Ugly Variable Names') degrade in Quality twice as fast as normal items", () => {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      // expected value: Quality decrease twice as fast
      const expectedValue = items[i].quality - 2;
